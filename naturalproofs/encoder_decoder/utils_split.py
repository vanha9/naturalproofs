...
def _tokenize_examples(split, dataset, tokenizer, rid2tok, title_only=False, conclusion_only=False):
    ...
    for eid, (tid, pix) in tqdm(enumerate(examples), total=len(examples)):
        ...
        title_ = '' if conclusion_only else ex['title']
        assumption_ = '' if title_only else ' '.join(replace_links(ex['assumption']))
        conclusion_ = '' if title_only else ' '.join(replace_links(ex['conclusion']))
        contents = "%s%s%s" % (
            title_,
            tokenizer.sep_token,
            assumption_,
...
if __name__ == '__main__':
    ...
    suffix += '' if not args.ex_conclusion_only else '_exconclusiononly'
